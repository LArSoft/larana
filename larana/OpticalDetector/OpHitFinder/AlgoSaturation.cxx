...
namespace pmtana {
    ...
    for (short const& value : wf) {
      ...
      if (fire && (double(value) < pre_threshold)) {
        ...
        if (record_hit && ((_pulse.t_end - _pulse.t_start) >= _min_width)) {
          ...
          if(std::find(_ignoreChannels.begin(), _ignoreChannels.end(), ch) == _ignoreChannels.end()){ CorrectSaturation(wf, _pulse, ch); }
  ...
  void AlgoSaturation::CorrectSaturation(pmtana::Waveform_t const& wf, pmtana::pulse_param& _pulse, int ch)
  {
      ...
      for (size_t i = start_tick; i <= static_cast<size_t>(_pulse.t_end); ++i) {//go over the full lenght of a pulse looking for saturation (slope ~ 0 near peak)
        if ((static_cast<double>(wf[i]) - static_cast<double>(wf[i - 1]) <= tolerance) && (static_cast<double>(wf[i]) >= _calibThrs)){//the last one is just a safegard against spurious fluctuations 
          if (!in_saturation) {
            in_saturation = true;
            satcounter = 1;
          }else{ satcounter++; }
      ...
      if (!plateau_lengths.empty()) {
        ...
        for(size_t i =0; i<plateau_lengths.size(); i++){
          double missingArea=calibvec[0]*pow(plateau_lengths[i],calibvec[1])*exp(calibvec[2]*plateau_lengths[i]);
          //for debugging 
          std::cout << "Saturated peak " << i << " ch " << ch << " old area " << _pulse.area << " sat ticks " << plateau_lengths[i] <<  " missing area " << missingArea << std::endl;
